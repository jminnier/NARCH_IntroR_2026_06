...
# !!!!!!!!UPDATE - Load files for today's workshop
...
-   **{tidyverse}**
    ...
    -   **[*Additional* {tidyverse}]{style="color:green"}** packages
        -   The [additional packages](https://tidyverse.tidyverse.org/#packages) are installed when the {tidyverse} package is installed, but they are not loaded when the {tidyverse} package is loaded. 
...
-   The `gt()` function is from the [`{gt}` package](https://gt.rstudio.com/)
...
[Image from [https://www.linkedin.com/posts/hadleywickham_rstats-activity-7435450431968919552-hY19](https://www.linkedin.com/posts/hadleywickham_rstats-activity-7435450431968919552-hY19)]{.smaller}
...
::: {style="font-size: 110%;"}
See the [`tabyl`s vignette](https://cran.r-project.org/web/packages/janitor/vignettes/tabyls.html){target="_blank"} for more `tabyl` and `adorn_` options:

[https://cran.r-project.org/web/packages/janitor/vignettes/tabyls.html](https://cran.r-project.org/web/packages/janitor/vignettes/tabyls.html){target="_blank"}
:::
...
-   Remember, since we haven't assigned this new "mutated" dataset to an object name, our work hasn't been saved.

## Using `mutate` to calculate a new variable based on other variables (2/3)

-   Below, we save the new dataset as `yrbss_new`
    -   to avoid overwriting our other data set.
    -   Check out the environment tab for the new dataset.
